...
(function (modules) { // webpackBootstrap
    ...
    ([
        ...
        (function (module, exports, __webpack_require__) {
            ...
            var bookmarksite = function bookmarksite() {
                var title = Adda247;
                var url = window.url;
            ...
            exports.bookmarksite = bookmarksite;
        ...
        (function (module, exports, __webpack_require__) {
            ...
            function attachDOMEventsToEventBus(eventBus) {
                ...
                var eventData;
                function createData() {
                    var fileName = getQueryStringValue('file').replace('.doc', '').split('_');
                    console.log(fileName);
                    return {
                        package_status: 'paid',
                        url: location.href,
                        childContent_id: fileName[1],
                        package_id: fileName[0],
                        user_action: 'ebook_opened',
                        package_type: 'ebook',
                ...
                function getCurrentRead() {
                    var cp_token = getCookie('cp_token');
                    if (cp_token === '')
                        return;
                    var fileName = getQueryStringValue('file').replace('.doc', '').split('_');
                    var xhr = new XMLHttpRequest();;
                    var url = location.origin + "/api/v1/ebooks/getStatusOfEbook/" + fileName[0] + "/" + fileName[1];
                    xhr.onreadystatechange = function () {
                        if (this.readyState == 4 && this.status == 200) {
                            var data = JSON.parse(this.responseText);
                            if (data.data)
                                PDFViewerApplication.page = data.data.onPageNumber;
                            else
                                syncCurrentRead(1);
                        }
                    };
                    xhr.open("POST", url, true);
                    xhr.setRequestHeader('X-JWT-Token', cp_token);
                    xhr.setRequestHeader('X-Auth-Token', 'fpoa43edty5');
                    xhr.setRequestHeader('Cache-Control', 'no-cache');
                    xhr.send();
                }

                function syncCurrentRead(pageNo) {
                    var cp_token = getCookie('cp_token');
                    if (cp_token === '')
                        return;
                    var fileName = getQueryStringValue('file').replace('.doc', '').split('_');
                    var xhr = new XMLHttpRequest();;
                    var url = location.origin + "/api/v1/ebooks/saveStatusOfEbook/" + fileName[0] + "/" + fileName[1] + "?pageNumber=" + pageNo;
                    xhr.open("POST", url, true);
                    xhr.setRequestHeader('X-JWT-Token', cp_token);
                    xhr.setRequestHeader('X-Auth-Token', 'fpoa43edty5');
                    xhr.setRequestHeader('Cache-Control', 'no-cache');
                ...
                eventBus.on('pagechange', function (e) {
                    ...
                    syncCurrentRead(e.pageNumber);
                    e.source.container.dispatchEvent(event);
                    var readProgerss = (e.pageNumber / e.source._pages.length) * 100;
                    var progressTick = Math.round(readProgerss / 10);
                    console.log(readProgerss);
                    if (progressTick > 0 && readProgerss % 10 <= 0.04) {
                        if (!eventData)
                            eventData = createData();
                        eventData.reached = (10 * progressTick) + "%";
                        Moengage.track_event("ebook_read", eventData);
                    }
        ...
        (function (module, exports, __webpack_require__) {
            ...
            window.print = function print() {
                abort();
                console.warn('Ignored window.print()');
                return;
            ...
            window.addEventListener('keydown', function (event) {
                if ((event.keyCode === 80 || event.keyCode == 83 || event.keyCode == 67) && (event.ctrlKey || event.metaKey) && !event.altKey && (!event.shiftKey || window.chrome || window.opera)) {
                    // window.print();
                    if (hasAttachEvent) {
                        return;
                    }
            ...
            if (hasAttachEvent) {
                document.attachEvent('onkeydown', function (event) {
                    ...
                    if ((event.keyCode === 80 || event.keyCode == 83 || event.keyCode == 67) && event.ctrlKey) {
                        event.keyCode = 0;
                        return false;
                    }
                });
